...
# Optional Polymarket browser trader (Chrome extension)
try:
    from .browser_polymarket import ChromeExtensionTrader
    CHROME_EXTENSION_TRADER_AVAILABLE = True
except Exception as e:
    ChromeExtensionTrader = None
    CHROME_EXTENSION_TRADER_AVAILABLE = False
    logger.warning(f"ChromeExtensionTrader unavailable: {e}")
...
""" + menu
        ...
        lines = [
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            "🗑️ **REMOVE HOLDING** (Select one)",
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            ""
        ]
        for i, h in enumerate(holdings, 1):
            lines.append(f"  **{i}** ➜ {h['ticker']}: {h['shares']:,.2f} shares")
        
        lines.append("")
        lines.append("  **0** ➜ Back to menu")
        lines.append("")
        lines.append("Reply with a number to remove:")
        lines.append("")
        lines.append("— Mamadou 💰")
    ...
    async def process_wizard(self, message: str) -> str:
        ...
        if step == "menu":
            if msg == "1":  # Add holding
                ...
                self.wizard_state = {}
                return "✅ Exited portfolio wizard\n\n— Mamadou 💰"
            else:
    ...
    async def quick_btc_direction(self) -> str:
        ...
        if not CHROME_EXTENSION_TRADER_AVAILABLE:
            return f"❌ Trading bot unavailable (Chrome extension not installed).\n\n— {self.emoji}"
        try:
            ...
            return f"❌ Trading bot failed: {str(e)}\n\n— {self.emoji}"
    ...
    async def chat(self, message: str, conversation_history: List[Dict[str, str]] = None) -> str:
        ...
        if any(phrase in msg_lower for phrase in ["start monitor", "monitor btc", "continuous", "watch btc"]):
            if "start" in msg_lower or "monitor" in msg_lower:
                ...
                try:
                    ...
                    return f"🔔 BTC monitoring started!\nWill alert on high-confidence signals.\n\n— {self.emoji}"
                except Exception as e:
                    return f"❌ Failed to start monitoring: {e}\n\n— {self.emoji}"

        if "stop monitor" in msg_lower:
            return f"🔔 BTC monitoring stopped.\n\n— {self.emoji}"

        if "chrome" in msg_lower or "extension" in msg_lower:
            return f"""🔗 **Chrome Extension Status**

For BTC trading bot:
1. Install Clawdbot Chrome extension
2. Click extension icon → badge should show "ON"
3. Navigate to Polymarket BTC 15m market
4. Say "up or down" to analyze

Extension enables:
• Live page scraping (no copy/paste)
...
— {self.emoji}"""
        ...
        if any(phrase in msg_lower for phrase in ["up or down", "btc direction", "polymarket", "btc 15m"]):
            if POLYMARKET_SKILL_AVAILABLE:
                return ("🎯 **Polymarket BTC 15m Direction Analysis**\n\n"
                ...
                return ("❌ Polymarket BTC 15m skill not installed.\n\n"
        ...
        if "portfolio" in msg_lower or "holdings" in msg_lower or "positions" in msg_lower:
            ...
            if holdings:
                ...
                for h in holdings:
                    lines.append(f"  • **{h['ticker']}**: {h['shares']:,.2f} shares ({h['type']})")
                lines.append("")
                lines.append("💡 Say **wizard** to manage your portfolio")
                self.log_action("portfolio_viewed", f"Showed {len(holdings)} holdings")
                return "\n".join(lines) + f"\n\n— Mamadou 💰"
            else:
                return "📊 Your portfolio is empty.\n\n💡 Say **wizard** to add holdings!\n\n— Mamadou 💰"
        
        # EdgeLab: Live quote scraping
        if any(phrase in msg_lower for phrase in ["live quote", "scrape price", "current price"]):
            if not EDGELAB_AVAILABLE:
                return "❌ EdgeLab adapters not available for live scraping.\n\n— Mamadou 💰"
            ...
            if tickers:
                ...
                if result.get("error"):
                    return f"❌ Failed to scrape {ticker}: {result['error']}\n\n— Mamadou 💰"
                
                return (f"📈 **Live Quote for {ticker}**\n\n"
                       f"• Price: {result.get('price', 'N/A')}\n"
                       f"• Change: {result.get('change', 'N/A')} ({result.get('change_pct', 'N/A')})\n"
                       f"• Source: {result.get('source', 'browser')}\n"
                       f"• Time: {result.get('timestamp', 'now')}\n\n"
                       "— Mamadou 💰")
            
            return "💡 Specify a ticker: `live quote AAPL`\n\n— Mamadou 💰"
        
        # EdgeLab: Polymarket markets
        if any(phrase in msg_lower for phrase in ["prediction market", "polymarket market", "betting odds"]):
            if not EDGELAB_AVAILABLE:
                return "❌ EdgeLab adapters not available.\n\n— Mamadou 💰"
            ...
            lines = [f"🎯 **Top Polymarket Markets{' - ' + category if category else ''}**\n"]
            for m in markets:
            ...
            lines.append("")
            lines.append("💡 Ask about sentiment: `polymarket sentiment Fed rate cut`")
            return "\n".join(lines) + "\n\n— Mamadou 💰"
            ...
            if len(parts) > 1:
                ...
                if keywords:
                    ...
                    if sentiment.get("error"):
                        return f"❌ Sentiment analysis failed: {sentiment['error']}\n\n— Mamadou 💰"
                    
                    lines = [f"🔮 **Polymarket Sentiment: {', '.join(keywords)}**\n"]
                    lines.append(f"• Markets Found: {sentiment.get('markets_found', 0)}")
                    
                    if sentiment.get('weighted_avg_probability') is not None:
                        prob = sentiment['weighted_avg_probability']
                        lines.append(f"• Weighted Probability: {prob*100:.1f}%")
                        lines.append(f"• Total Volume: ${sentiment.get('total_volume', 0):,.0f}")
                        
                        if sentiment.get('markets'):
                            lines.append("\n**Top Markets:**")
                            for m in sentiment['markets'][:3]:
                                lines.append(f"• {m.get('question', 'N/A')[:50]}...")
                                lines.append(f"  → {m.get('yes_price', 0)*100:.1f}% likely")
                    
                    return "\n".join(lines) + "\n\n— Mamadou 💰"
            
            return "💡 Specify keywords: `polymarket sentiment Fed rate cut`\n\n— Mamadou 💰"
        
        # Help
        if "help" in msg_lower:
            help_text = """💰 **Mamadou - Finance Agent**

**Portfolio Wizard:**
• Say **wizard** to open the interactive portfolio manager

**Quick Commands:**
• `show portfolio` - View holdings
• `add 100 AAPL` - Quick add
• `remove AAPL` - Quick remove"""
            
            if EDGELAB_AVAILABLE:
                help_text += """

**EdgeLab Data:**
• `live quote AAPL` - Scrape live price
• `prediction market crypto` - Top Polymarket markets
• `polymarket sentiment BTC halving` - Crowd wisdom"""
