...
## 🔧 Coding Conventions (from .learnings)

### Database Model to API Field Mapping
When SQLAlchemy model fields differ from API schema fields, be explicit:

```python
# In model (database/models.py):
preview = Column(Text)  # API exposes this as "body"

# In API serialization - CORRECT:
"body": m.preview or "",  # Map DB field to API field name

# WRONG - accessing non-existent attribute:
"body": m.body or "",  # Will cause AttributeError
```
